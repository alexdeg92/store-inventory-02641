...
interface Props {
  onCount: (productId: number, count: number) => void; // apply a count update
}
...
export default function ScannerTab({ onCount }: Props) {
  ...
  const resolvedProduct = result?.productId != null ? PRODUCTS.find(p => p.id === result.productId) : null;
    ...
    <div className="flex flex-col items-center px-4 py-6 gap-6 max-w-md mx-auto">
      ...
      {scanning && (
        <div className="w-full relative">
          <div id="qr-reader" ref={scannerRef} className="w-full rounded-2xl overflow-hidden border-4 border-blue-400 shadow-xl" />
          <button
      ...
      {/* Error */}
      {error && <div className="w-full bg-red-50 border border-red-200 rounded-2xl p-4 text-red-700 text-sm">{error}</div>}
        ...
        <div className="w-full bg-gray-50 border border-gray-200 rounded-2xl p-4 flex flex-col gap-4">
          ...
          {resolvedProduct ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-3">
              ...
              {resolvedProduct.priceCategory > 0 && <p className="text-sm text-gray-500">{resolvedProduct.priceCategory}$ • Emb. {resolvedProduct.packQty}</p>}
            </div>
          ) : (
